...
- **Service Object 禁止**: `app/services/` は作成しないこと。ビジネスロジックはモデルのクラスメソッド（例: `Tenant.signup!`）に書き、コントローラは薄く保つこと。エラーハンドリングは `rescue ActiveRecord::RecordInvalid` をコントローラで行う。
